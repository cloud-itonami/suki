(ns suki.methods.social
  "social.cljc — 鋤 — Farm Tractor Manufacturing DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from suki's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "鋤 — Farm Tractor Manufacturing が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over suki's own domain (Farm tractor manufacturing Tier-B actor. Manufacturing-side counterpart of mitsuho operator-side (ADR-2605261015), parallel to…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "suki R0: live social posting is Council Lv6+ + operator + "
